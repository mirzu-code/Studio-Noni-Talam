...
const Checkout = () => {
  ...
  const generateWhatsAppMessage = (orderId) => {
    ...
    // Retry insert (max 3 attempts) and request the inserted record
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        alert(`Failed to save booking: ${err?.message || err}`);
        setIsProcessing(false);
        return;
      }
    }
                ...
                <div style={{ background: '#F8FAFC', padding: '2rem', borderRadius: '8px', border: '1px solid #E2E8F0', marginBottom: '2rem', textAlign: 'center' }}>
                  {paymentMethod === 'qr' && (
                    <>
                      ...
                      <img src="gambar qr.jpeg" alt="QR Code" style={{ width: '200px', margin: '1rem auto', borderRadius: '8px', border: '4px solid white', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }} />
